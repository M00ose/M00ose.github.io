...
const Projects = (props) => {
  return (
    <motion.div 
      ...
      initial="hidden"
      whileInView="show"
      className={`relative snap-start h-screen w-screen overflow-scroll ${styles.padding} flex flex-col justify-center items-center md:items-start`}
    >
        ...
        <div className={`absolute inset-y-2/4 md:inset-y-1/4 md:inset-x-1/3 h-[45vh] md:h-[50vh] w-[90vw] md:w-[60vw] overflow-hidden drop-shadow-xl`}>
          <motion.div 
            variants={fillIn("right","tween",1,1)} 
            className={`h-full w-full bg-white`}></motion.div>
        </div>

        <div className={`absolute flex flex-col gap-4 inset-y-2/4 md:inset-y-1/4 md:inset-x-1/3 ${styles.padding} h-[40vh] md:h-[50vh] w-[90vw] md:w-[60vw] z-20`}>
        ...
        <motion.div 
            ...
            className={`absolute md:left-[28%] inset-y-[48%] md:inset-y-[23%] min-w-[300px] h-fit w-fit p-2 bg-tertiary ${styles.border} z-20 text-center`}>{props.name}
        </motion.div>
